...
class Lambda
{
	...
	virtual void operator() (T const& x)
	{
...
public:
	...
	virtual void apply(FuzzyFunctorMem const& ff, S const& x)
	{
		func(x,ff);
	}
...
template<class F>
std::mem_fun_t<void,F> make_functor(std::mem_fun_t<void,F> f, void *b, void *c) {
	mem_fun(&F::func)();
};
